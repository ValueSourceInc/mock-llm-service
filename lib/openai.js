...
export async function handleChat(body, res, ov) {
  ...
  // token 上限取值链: mock_max_tokens(直打 mock 用,过不了中转站) ?? max_tokens(标准字段,能穿透 new-api relay) ?? 默认值
  const targetTokens = body.mock_max_tokens ?? body.max_tokens ?? config.defaultOutputTokens;
